...
--  CLOUD version of schema.sql
-- ============================================================
--  Use this file on a hosted database (Railway, PlanetScale,
--  Aiven, Clever Cloud...).
--
--  It differs from schema.sql in ONE important way: it does NOT
--  contain DROP DATABASE / CREATE DATABASE / USE.
--
--  Why: a cloud provider creates the database FOR you and gives
--  you a user that can only work INSIDE it. That user is not
--  allowed to create or drop databases, so schema.sql would stop
--  at line 7 with "Access denied". This file creates only the
--  TABLES, inside whichever database you are already connected to.
--
--  Railway calls that database "railway" by default. That name is
--  fine - the table names are what the Java code cares about.
-- ============================================================

-- Dropped children-first so the foreign keys never block a drop.
